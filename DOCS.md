...
A Service Bus instance was successfully set up. Ensure the @azure/service-bus NPM package is installed: `npm install @azure/service-bus`. To set up an instance, see the `service-bus.js` file. The `connectionString` is the primary connection string which is obtained from the Azure Portal: portal > search for Service Bus > select Service Bus Namespace > Settings > Shared access policies > RootManageSharedAccessKey > copy Primary Connection String. The name of the Service Bus queue can be found by finding the Service Bus Namespace, opening the Overview tab, scrolling to the bottom, and then copying the name of the queue itself.
