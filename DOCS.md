...
~~I did also set up a `.env` just to try and use environment variables, but I got back errors from both the node-postgres and Sequelize packages saying the password had to be a string (will need to come back and look into this more).~~ Setting up a schema to use environment variables via Docker Compose was achieved, see the [Environment Variables](https://github.com/rtasalem/hapi-pg-docker/blob/main/DOCS.md#environment-variables) section for full details.
